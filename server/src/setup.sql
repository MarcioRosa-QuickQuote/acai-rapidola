...
-- Histórico de ganhos dos motoboys (gerado ao marcar pedido como entregue)
...
-- Histórico de ganhos das lojas (gerado ao marcar pedido como entregue)
...
-- Colunas adicionadas ao users (rodar se necessario):
...
-- Coluna delivery_fee na tabela orders (rodar se necessario):
ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery_fee REAL DEFAULT 0;

-- Coluna pix_key nas lojas (rodar se necessario):
ALTER TABLE stores ADD COLUMN IF NOT EXISTS pix_key TEXT DEFAULT '';

-- Colunas adicionadas ao stores (rodar se necessario):
